#!/usr/bin/env python3
"""Gate B10 Task C+D analysis: C4 vs C0 at iters in {8,12,16,20}.

Statistic of record: median. Test of record: Mann-Whitney U, two-sided.
Correction: Holm-Bonferroni across the 4 pre-registered comparisons
(GATE_B10_REPLICATION_PREREGISTRATION.md Section 4). Kept reps only.
"""
import csv
...
CSV_PATH = "t_b10c_replication_crossover_times.csv"
...
for it in iters_list:
    ...
    med_c0 = np.median(c0)
    med_c4 = np.median(c4)
    pct = (med_c4 - med_c0) / med_c0 * 100.0
...
print("=== Gate B10 Task C+D: C4 vs C0, kept reps only, n=10/arm/iters ===")
...
mde_map = {8: 2.49, 12: 2.49, 16: 2.49, 20: 1.85}
with open("t_b10c_aggregate_comparison.csv", "w", newline="") as f:
    w = csv.writer(f)
...
with open("t_b10c_atomics_summary.csv", "w", newline="") as f:
    w = csv.writer(f)
