...
**Superseded, kept for provenance -- see `STENCIL_LABEL_COLLISION.md` and `GATE_T3_REPORT.md` for the current recommendation.** The "provenance caveat" this report could only flag has since been diagnosed: the 1645.6ms G3 figure used below is the **kernel-loop-time** convention (`run_robust.py`'s internal timer), not the **process-wall-clock** convention (`lib_specasync_harness.sh`) used by every C0-C3 table in this project, including `gate3_times.csv`. The two are not comparable, which is exactly why they disagreed by ~2.5x below. ~~The manuscript should use Task T3's Sweep-24K point (15.31%, process-wall-clock basis, same N=24000 workload) in place of the 7.86% figure computed in section 4 here.~~ **UPDATE (2026-08-13): that recommendation is now itself superseded.** `CEILING_BASIS_VERIFICATION.md` found that `GATE_T3_REPORT.md`'s ceiling formula (and this project's Gate B7 5070 Ti rerun, which copied it) has a units-mismatch bug that inflates ceilings by ~5x -- T3's Sweep-24K point is corrected to 7.43% (T4) / 7.25% (5070 Ti), not 15.31%. **This report's own 7.863% figure (Section 4 below) does NOT carry that bug** -- its numerator was a single canonical decomposition capture, never aggregated across multiple trials, so the trial-count mismatch that inflated T3's numbers by ~5x never applied here. This section's 7.86% and T3's corrected ~7.3-7.4% for the same underlying workload (N=24000 `bench_stencil`) now agree closely, despite being computed independently, under different wall-clock conventions, from different capture sessions -- **this is corroborating, not contradicting, evidence for the corrected convention**, even though this figure's own kernel-loop-vs-process-wall-clock provenance caveat (Section 2/3 below, `STENCIL_LABEL_COLLISION.md`) still stands separately and is not resolved by this agreement.
...
**This computed value (7.86%) is not affected by the units-mismatch bug `CEILING_BASIS_VERIFICATION.md` found in `GATE_T3_REPORT.md`'s later 6-workload formula** (see the banner at the top of this file) -- its numerator (`sum(total_ns)` over 5,253 batches) came from one canonical decomposition capture, not summed across multiple trials, so no trial-aggregation/single-median mismatch was possible here. It independently agrees with T3's corrected Sweep-24K figures (7.43% T4 / 7.25% 5070 Ti) to within a point, despite different wall-clock conventions and capture sessions -- corroboration for the corrected convention, offered here for the record.
